...
#define DEBUG_PRINTF(fmt, ...) printf("Websocket: " fmt "", ##__VA_ARGS__);
  ...
  if (data != NULL && len > 1) {
    ...
    switch (opcode) {
      ...
      case 0x02: {
        ...
        memcpy(this->data, payload, strlen((char *)payload)+1);
        on_recv();
        break;
      }
      case 0x08:
...
void WebSocket::on_connected() {
  ...
  DEBUG_PRINTF("Connected to %s at port %d\n", ipstring, ser_port);
...
void removeWebsocket(WebSocket *websocket) {
  switch (websocket->state) {
    ...
    case WebSocket::State_t::CONNECTED:
      ...
      break;
    default:
      DEBUG_PRINTF("This should not be here. STATE %d\n", websocket->state);
  }
}
